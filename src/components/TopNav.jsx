...
() {
  ...
  return (
    <div className='max-w-[1520] mx-auto flex justify-between items-center p-4'>
        ...
        <button className='bg-orange-700 text-white hidden md:flex items-center py-2 rounded-full'>Cart
        <BsFillCartFill size={20}/>
        </button>     
       {
        sideNav ? (
            <div className='bg-black/60 fixed w-full h-screen zi10 top-0 left-0' onClick={()=>setSideNav(!sideNav)}></div>
        ):("")
       }
       <div className={sideNav ?'fixed top-0 left-0 w-[300px] h-screen bg-white z-10 duration-200':'fixed top-0 left-[-100%] w-[300px] h-screen bg-white z-10 duration-300' }>
